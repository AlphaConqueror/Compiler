...
public class Parser {
	...
	private Expression acceptAnnotatedExpression(TokenKind kind) {
		if (accept(kind)) {
			expect(TokenKind.LPAREN);
			Expression res = parseExpression();
			expect(TokenKind.RPAREN);
	...
	private Statement parseWhile() {
		...
		Expression invariant = acceptAnnotatedExpression(TokenKind._INVARIANT);
		Expression term = acceptAnnotatedExpression(TokenKind._TERM);
		final Statement body = parseInnerStatement();
		return factory.createWhileStatement(loc, cond, body, invariant, term);
	}
